...
def _compose_author_notes(data: dict[str, Any]) -> str:
    parts: list[str] = []
    if data.get("working_title"):
        parts.append(f"Working title: {data['working_title'].strip()}")
    if data.get("premise"):
        parts.append(f"Premise: {data['premise'].strip()}")
    if data.get("setting"):
        parts.append(f"Setting: {data['setting'].strip()}")
    parts.append(f"Opening situation: {data['opening_situation'].strip()}")
    facts = data.get("initial_observable_facts") or []
    if facts:
        parts.append("Initial observable facts:")
        parts.extend(f"- {str(item).strip()}" for item in facts)
    if data.get("author_notes"):
        parts.append(str(data["author_notes"]).strip())
    return "\n".join(parts).strip()
...
def map_semantic_to_canonical(data: dict[str, Any]) -> dict[str, Any]:
    ...
    brief["author_notes"] = _compose_author_notes(data)
    return brief
